...
with tf.Graph().as_default():
	...
	with chn_sess.as_default():
		...
		while True:
			...
			logits, trans_params= chn_sess.run([chn.ner_project_logits,chn.ner_trans_params],feed_dict)
			...
			at1 = np.divide(np.sum(at1, axis=(1,2)), 640)
			at2 = np.divide(np.sum(at2, axis=(1,2)), 640)
			...
			for i in range(len(msg)):
				if(ans[i][0] == 'B'):
					...
					state = ans[i][2:]
					restext = restext + '<mark data-entity="' + state + '">'
				elif(ans[i] == 'O'):
					if state != '':
						restext = restext + '</mark>'
					state = ''
