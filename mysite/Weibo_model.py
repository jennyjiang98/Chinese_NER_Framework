...
class TransferModel(object):
    ...
    def self_attention(self,keys, scope='multihead_attention', reuse=None, mode='ner'):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            #print(outputs.shape)
            if(mode=='ner'):
                self.ner_attention_weight = tf.reshape(outputs, [-1, self.num_heads, self.num_steps, self.num_steps])
            elif(mode=='shared'):
                self.shared_attention_weight = tf.reshape(outputs, [-1, self.num_heads, self.num_steps, self.num_steps])
    ...
    def multi_task(self):
        ...
        with tf.variable_scope('common_bilstm'):
            ...
            shared_output=self.self_attention(shared_output, mode='shared')
        ...
        with tf.variable_scope('cws_private_bilstm'):
            ...
            cws_private_output=self.self_attention(cws_private_output, mode='cws')
        ...
        with tf.variable_scope('ner_private_bilstm'):
            ...
            ner_private_output=self.self_attention(ner_private_output, mode='ner')
